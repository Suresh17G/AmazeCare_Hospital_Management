...
public class AdminServiceImpl implements IAdminService {
	...
	@Override
	public List<PrescriptionDetailsDTO> viewAllPrescriptions() {
		logger.info("Request initiated to view all prescriptions");
		List<Prescription> list = prescriptionRepo.findAll();
		if(list.isEmpty()) {
			logger.warn("No Prescriptions found!");
			return null;
		}
		return list.stream().map(i->modelMapper.map(i,PrescriptionDetailsDTO.class)).toList();
	}
	...
	@Override
	public AppointmentDetailsDTO editAppointment(int appointmentid, AppointmentDTO app) {
		logger.info("Request initiated to edit appointment with id: "+appointmentid);
		Appointment existingAppointment= appointRepo.findById(appointmentid).orElse(null);
	    ...
	    if(app.getAppointmentDate()!=null) {
	    	existingAppointment.setAppointmentDate(app.getAppointmentDate());
	    }
	    if(app.getAppointmentTime()!=null) {
	    	existingAppointment.setAppointmentTime(app.getAppointmentTime());
	    }
	    if(app.getStatus()!=null) {
	    	existingAppointment.setStatus(app.getStatus());
	    }
	    if(app.getReason()!=null) {
	    	existingAppointment.setReason(app.getReason());
	    }
	    if(app.getVisitType()!=null) {
	    	existingAppointment.setVisitType(app.getVisitType());
	    }
	    if(app.getConsultationType()!=null) {
	    	existingAppointment.setConsultationType(app.getConsultationType());;
	    }
	    
	    Appointment updated=appointRepo.save(existingAppointment);
