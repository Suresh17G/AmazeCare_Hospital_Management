...
public class DoctorServiceImpl implements IDoctorService{
	...
	public List<PrescriptionDetailsDTO> viewPrescriptionsByRecordId(int recordId) {
  
        List<Prescription> prescriptions = prescriptionRepo.findByMedicalRecord_recordId(recordId);
   
        if (prescriptions == null || prescriptions.isEmpty()) {
            return null; // You could also return an empty list if you prefer
        }

   
        return prescriptions.stream()
                .map(prescription -> modelMapper.map(prescription, PrescriptionDetailsDTO.class))
